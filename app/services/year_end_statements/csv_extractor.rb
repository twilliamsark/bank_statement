  ...
  class CSVExtractor
    ...
    def build_categories(rows)
      ...
      rows.each do |row|
        ...
        category = (grouped_categories[transaction.category] ||= { total: 0.to_d, subcategories: {} })
        category[:total] += transaction.amount

        subcategory = (category[:subcategories][transaction.subcategory] ||= { total: 0.to_d, transactions: [] })
